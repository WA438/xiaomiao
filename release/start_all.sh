...
# ═══════════════════════════════════════════════
# 第3.5步：清理旧固件文件（只保留最新版本）
# ═══════════════════════════════════════════════
echo "[3.5/8] 清理旧固件文件..."
FW_DIR="$RELEASE_DIR/firmware"
if [ -d "$FW_DIR" ]; then
    LATEST_FW=$(ls -t "$FW_DIR"/xiaomiao_V*.bin 2>/dev/null | head -1)
    if [ -n "$LATEST_FW" ]; then
        OLD_COUNT=0
        FREED_BYTES=0
        ...
        if [ "$OLD_COUNT" -gt 0 ]; then
            ...
            echo "  删除 $OLD_COUNT 个旧固件 (释放 ${FREED_KB}KB)"
        else
            echo "  无旧固件需清理"
        fi
        echo "  保留: $(basename "$LATEST_FW")"
    fi
fi

# 清理临时文件
for f in "$RELEASE_DIR"/*.tmp "$RELEASE_DIR"/*.tmp.*; do
    [ -f "$f" ] && rm -f "$f"
done
echo "  临时文件已清理"
